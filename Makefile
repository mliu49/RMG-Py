...
minimal:
	python setup.py build_ext minimal --build-lib . --build-temp build

main:
	@ echo "Checking you have PyDQED..."
	@ python -c 'import pydqed; print pydqed.__file__'
	python setup.py build_ext main --build-lib . --build-temp build
...
ifneq ($(DASPK),)
	...
	python setup.py build_ext solver --build-lib . --build-temp build

arkane:
	python setup.py build_ext cantherm --build-lib . --build-temp build
...
ifeq ($(OS),Windows_NT)
	@ -rd /s /q build\cython\rmgpy\solver
	@ -rd /s /q build\build\rmgpy\solver
else
	@ -rm -r build/cython/rmgpy/solver/
	@ -rm -r build/build/rmgpy/solver/
endif
	@ echo "Removing compiled files..."
